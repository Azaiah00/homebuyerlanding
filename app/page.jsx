...
export default function HomePage() {
  ...
  const quizQuestions = [
    {
      id: 1,
      question: "What's the key difference between being Pre-Qualified and Pre-Approved?",
      options: [
        "Pre-Qualified is a quick estimate, Pre-Approved is a thorough verification",
        "Pre-Approved is only for cash buyers",
        "They are essentially the same thing",
        "Pre-Qualified requires more documentation"
      ],
      correctAnswer: 0,
      hint: "Pre-Qualified is a quick 7-minute estimate based on basic information. Pre-Approved is a thorough qualification with document verification that makes you a serious buyer and lets you act fast. Pre-Approval is required to be competitive in today's market!"
    },
    {
      id: 2,
      question: "What is the minimum down payment you typically need to buy a home?",
      options: [
        "20%",
        "10%",
        "As little as 3%",
      ...
      correctAnswer: 2,
      hint: "Many loan programs allow you to buy a home with a down payment as low as 3% or 3.5%. While 20% is ideal to avoid PMI, it's not always required to get started!"
    },
    {
      id: 3,
      question: "What is the typical range for an Earnest Money Deposit (EMD) in a home purchase?",
      options: [
        "0.5% - 1% of the purchase price",
        "1% - 5% of the purchase price",
        "A fixed amount of $500",
        "10% or more of the purchase price"
      ],
      correctAnswer: 1,
      hint: "The Earnest Money Deposit (EMD) shows the seller you're serious. It's typically held in an escrow account and applied towards your down payment or closing costs at settlement. A common range is 1% to 5% of the home's purchase price, with 3% being standard in many competitive markets."
    },
    {
      id: 4,
      question: "Approximately how much should you budget for closing costs when buying a home?",
      options: [
        "0.5% - 1% of the loan amount",
        "2.5% - 3% of the home's purchase price",
        "A flat fee of $1,000",
        "5% - 7% of the home's purchase price"
      ],
      correctAnswer: 1,
      hint: "Closing costs are fees associated with finalizing your mortgage and home purchase. They typically include lender fees, title insurance, appraisal fees, and more. A good rule of thumb is to budget 2.5% to 3% of the home's purchase price for these expenses, though it can vary."
    }
  ]
  ...
  // Tooltip positioning handler - aligns with seller consultation but keeps viewport safety
  useEffect(() => {
    const measureTooltip = (text, type) => {
      ...
      el.style.left = '0'
      el.style.top = '0'
      el.style.padding = type === 'icon' ? '0.75rem 1rem' : '1.25rem 1.5rem'
      el.style.maxWidth = type === 'icon' ? '320px' : '320px'
      el.style.minWidth = type === 'icon' ? '220px' : '220px'
      el.style.fontSize = type === 'icon' ? '0.85rem' : '0.95rem'
  ...
  const handleSubmit = async (e) => {
    ...
    // IMPORTANT: Add your API key to .env file: NEXT_PUBLIC_BREVO_API_KEY=your_key_here
    ...
    if (!BREVO_API_KEY) {
      ...
      console.error('For local development: Add NEXT_PUBLIC_BREVO_API_KEY to your .env file')
      console.error('For production: Add NEXT_PUBLIC_BREVO_API_KEY in Netlify Dashboard → Site Settings → Environment Variables')
